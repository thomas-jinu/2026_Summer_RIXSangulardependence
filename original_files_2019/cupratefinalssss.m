...
12/6/19     First plots complete. The pi problem persists
            Also convolution with the Gaussian remains.
...
for i=1:1:n
    for d_out=1:1:5
        ...
        w(:,i) = -w_in;
        %cs_f (d_out,s_out,pol_in,:)
        f1=repmat(squeeze(cs_f(d_out,1,2,:)),1,n);
        f2=repmat(squeeze(cs_f(d_out,2,2,:)),1,n);
        f3=repmat(squeeze(cs_f(d_out,1,1,:)),1,n);
        f4=repmat(squeeze(cs_f(d_out,2,1,:)),1,n);
        
        I_s(:,i)= I_s(:,i) + (Gamma(d_out,1)/pi) .* ( f1(:,i)./((w(:,i) + E_f(d_out,1)).^2 + Gamma(d_out,1).^2) ...
                                                    + f2(:,i)./((w(:,i) + E_f(d_out,1) + 2*J_f(d_out)).^2 + Gamma(d_out,1)^2));
    
        I_p(:,i)= I_p(:,i) + (Gamma(d_out,1)/pi) .* (f3(:,i)./(( w(:,i) + E_f(d_out,1)).^2 + Gamma(d_out,1)^2) ...
...
norm_s=zeros(numel(delta),1);

figure;
for ang=3:1:numel(delta)-2
norm_s(ang,1)=sum(I_s(ang,:),2)
plot(w(ang,:), I_s(ang,:)+ .01*ang,'r-', 'linewidth', 1.0);
hold on;
end
...
norm_p=zeros(numel(delta),1);

figure;
for ang=3:1:numel(delta)-2
norm_p(ang,1)=sum(I_p(ang,:),2)
plot(w(ang,:), I_p(ang,:)+ .01*ang,'r-', 'linewidth', 1.0);
hold on;
set(gcf,'color','white')
end
xlabel('Energy Loss (eV)');
